...
export const createComment = async (req, res, next) => {
    try{
        ...
        if(userId !== req.user.id){
            return next(errorHandler(403, 'You are not allow to comment on this post'))
        }
...
export const getPostComments = async (req, res, next) => {
    try{
        const comments = await Comment.find({postId: {postId: req.params.postId} }).sort({createdAt: -1});
